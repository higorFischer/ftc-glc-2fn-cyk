...
export class CNF extends GLC {
	...
	private fromGLC(glc: GLC) {
        ...
        */
		...
		newGLC.variables = { F: ["S"], ...glc.variables };
        ...
        */
		for (var variable of Object.keys(glc.variables).reverse()) {
			...
			newGLC.variables[variable] = glc.variables[variable].filter(
				(c) => c !== ""
			);
		}
			...
			for (var rule of newGLC.variables[variable]) {
				if (rule.length > 2) {
					for (var i = 1; i < rule.length - 1; i++) {
						var oldRuleIndex =
							newGLC.variables[variable].indexOf(rule);
						...
						newGLC.variables[variable].splice(
							oldRuleIndex,
							1,
		...
		for (var variable of Object.keys(newGLC.variables)) {
			for (var rule of newGLC.variables[variable]) {
				if (newGLC.isTerminalAndValue(rule)) {
					...
					newGLC.variables[variable][oldRuleIndex] = rule.replace(
						terminal,
						newKey
					);
				}
